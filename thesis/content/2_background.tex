...
\subsection{Why not use Wi-Fi, ZigBee, LTE/4G, etc.? - Problems with established wireless communication techniques}

% show problems with current, widely used wireless communication techniques such as Wi-Fi, LTE, etc. 

While there are sensors that can be connected to the internet using Wi-Fi, ZigBee, LTE/4G and other, these technologies are not suitable for all use cases.

%  todo add figure with bitrate, range, power consumption, etc. for different technologies
...
LTE/4G not suitable for battery-powered devices either as it requires a lot of power.
...
The most widely used methods, called \emph{backhauls}, to realize this are Ethernet/LAN, Wi-Fi and 4G/LTE connections.
...
While gateways like these usually have a built-in antenna, their range is usually only sufficient for covering small to medium sized buildings or areas.
...
\ac{LBS} uses TLS-encrypted \ac{TCP} connections with token-based authentication to relay \ac{LoRaWAN} packets to the \ac{LNS}~\cite{the_things_industries_bv_lora_2022}.
...
The gateways being used in this thesis were mainly ones that were already in the possession of the \ac{HFU}.

One notable exception is the two MikroTik wAP LR8 kit gateway that was purchased for the thesis.
It was chosen after recommendation by a member of the HSN-TTN team.
...
\acl{TTNM} works by using \ac{LoRaWAN} nodes with GPS modules that send their location to the \ac{TTN} network via uplink messages.
...
Hence, \acl{TTNM} can use the information about which gateways received the message and the location information of the nodes' GPS modules to build a map displaying the coverage of \ac{LoRaWAN} gateways in the \ac{TTN} network.

% explain why it is important for the thesis

As far as this thesis is concerned, \acl{TTNM} is important because it provided a way to access \ac{RSSI} and location data points for measurements done by GPS-enabled \ac{LoRaWAN} nodes.