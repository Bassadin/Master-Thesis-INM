...
A \ac{LoRa} gateway is a device that receives \ac{LoRa} packets (usually from \ac{LoRa} devices) and forwards them to the \ac{LoRaWAN} server.
...
However, it is also possible to use external antennas if it is necessary to cover a much larger area as was the case in this thesis since a medium to long range localization of \ac{LoRa} devices was the goal.
...
As far as this thesis is concerned, \acl{TTNM} provides a way to access \ac{RSSI} and location data points for measurements done by \ac{GPS}-enabled \ac{LoRaWAN} devices.
