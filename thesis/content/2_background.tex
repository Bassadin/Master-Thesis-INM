...
A \ac{LoRa} node that transmits \SI{75}{\byte} of payload data every 2 hours can theoretically last up to 10 years on a single 1000 mAh battery~\cite{cheong_comparison_2017-1}.
...
Its source code, named \ac{TTS}, is also available on GitHub under the Apache 2.0 license~\cite{noauthor_thethingsnetworklorawan-stack_2023}.
...
It is not used in this thesis, so it will not be explained in more detail.
...
This will be explained in more detail in \cref{sec:ttm-data-collection}.
...
\begin{figure}
    ...
    \caption{Example of how multilateration works in \ac{GNSS} in 2D and 3D.\protect\cite{gisgeography_how_2018}}\label{pic:multilateration-gps-2d-3d-example}
\end{figure}
...
\begin{figure}
    ...
    \caption{Example of multilateration in 2D with error ranges. Contrary to \cref{pic:multilateration-gps-2d-3d-example}, the margins of error can be seen as well as the non-definitive margin of error around the estimated device position.\protect\cite{kapoor_novel_2016}}\label{pic:multilateration-with-error-ranges-example}
\end{figure}
...
% TODO explain what an annulus is? lol
\Cref{pic:multilateration-with-error-ranges-example} shows an example of this in 2D where the device's position is estimated to be in the area where the three annuli overlap.
