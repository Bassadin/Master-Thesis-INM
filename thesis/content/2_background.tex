...
Because of the correlation between \ac{SF} and \ac{SNR}, it is important to use both in conjunction.
...
Each end device usually has a unique 128-bit \ac{AES} key that is used to encrypt the payload~\cite{lora_alliance_inc_lorawan_2017-1}.

This \ac{E2EE} also makes \ac{LoRaWAN} packets secure against replay attacks, since a so-called \ac{MIC} is determined from the frame data, which in turn contains a \emph{DevNonce}, a number that is incremented with each packet sent~\cite{lora_alliance_inc_lorawan_2017-1}.
...
To show the relevance of using \ac{RSSI} values over \ac{GNSS} systems to locate end devices, this chapter will give a brief comparison of power usage between the two.
The \emph{ELV-LW-GPS1} module is a \ac{GPS} tracker that was used for data collection during this thesis.
It will be evaluated in more details in \cref{subsubsec:elv-gps-tracker-implementation}.
