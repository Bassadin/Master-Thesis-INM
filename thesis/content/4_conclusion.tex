...
\begin{figure}[htbp]
    ...
    \caption{
        Screenshot of a \ac{TTNM} map showing some data points taken from the \ac{LoRaWAN} gateway deployed on top of the \ac{GHB} student dormitory.
...
\begin{figure}[htbp]
    ...
    \caption{
        ...
        The range to the city of Furtwangen is almost nonexistent due to the hills in the way~\cite{ttn_mapper_ttn_2023}.
...
\begin{figure}[htbp]
    ...
    \caption{
        ...
        This is due to the fact that several \ac{LoRaWAN} gateways were deployed during this thesis as well as the additional data collection performed with \ac{LoRaWAN} \ac{GPS} trackers~\cite{ttn_mapper_ttn_2023}.
...
\ac{kNN} also uses a database of fingerprints.
...
\subsubsection{Use \ac{SF} and \ac{SNR} values for fingerprinting}
...
However, since \ac{TTN} captures all of those values with eack received data frame, this would not be a problem other than the increased amount of data that would need to be stored.
