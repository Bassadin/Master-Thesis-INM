...
\Cref{pic:dl0fis_gw_range} shows the coverage of the \acl{LRWGW} deployed on the roof of the \emph{DL0FIS} clubhouse.
...
\Cref{subsec:toa-based-multilateration-implementation} mentioned the fact that using timestamps from a \acl{LRWGW} to determine the range to the \acl{ED} is too inaccurate to be used for \ac{ToA}-based geolocation.
...
As most \ac{LoRaWAN} \aclp{ED} used in this thesis were using \ac{SF}7, the example given in \cref{fig:fingerprinting-map-example-with-snr-and-sf} doesn't show much improvement of accuracy over \cref{fig:fingerprinting-map-example-with-snr}.
...
From installing the \ac{LoRaWAN} gateways to collecting the data via \aclp{ED}, as well as sending it to \ac{TTN} and \ac{TTNM}, to visualizing it on a website that was deployed in a containerized environment along with a backend service and a \ac{DB}.
