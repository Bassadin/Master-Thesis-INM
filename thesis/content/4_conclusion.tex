...
\Cref{fig:fingerprinting-map-example-only-center} already showed an example where the center of matching filtered datapoints was taken as the estimated position, along with an arbitrary radius to show the associated uncertainty.
...
\begin{figure}[htbp]
    ...
    \caption{
        ...
        The yellow circle shows the estimated position that \ac{TTNL} calculated out of the center of the matching datapoints.
...
This usually leads to a larger margin of error (low precision) but a higher rate of actually containing the actual position (high accuracy).
