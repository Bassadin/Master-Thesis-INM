...
\Cref{lst:sql-calculate-correlation-coefficient} shows the \ac{SQL} query that was used to calculate the correlation coefficient between the \ac{RSS} values and the associated distances.
...
% TODO this is bullshit because gateways differ, do it for one gateway as an example

To remove outliers, only gateways with at least \num{500} associated \ac{RSS} values were considered in the calculation.

\subsubsection{Accuracy of \acs{RSSI}-based multilateration with a fixed \acs{RSSI} to range function}\label{subsubsec:conclusion-rssi-fixed-scale}
...
% TODO: did this make things better?
...
\subsubsection{Meshing of \acs{LoRaWAN} gateways}

% TODO mention how this could be possible, also mention Anton Petrov's thesis
