...
While \ac{LoRa} \aclp{ED} can feature \ac{GPS} receivers to geolocate them when deployed, they consume considerable amounts of power, especially when trying to obtain a location fix.
...
The goal of this thesis is to analyze and compare \ac{GPS}-less methods for geolocating standard \ac{LoRaWAN} devices without the need to modify those \aclp{ED} or the gateways used.
...
To achieve this goal, some \ac{LoRaWAN} gateways that were already registered in \acf{TTN} in Furtwangen were used as well as new gateways placed in Furtwangen.
...
They did not use the \ac{LoRaWAN} protocol but instead used a small test setup of their own \ac{LoRa} \aclp{ED}.
