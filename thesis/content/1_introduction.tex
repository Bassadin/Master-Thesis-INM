...
However, bridging long distances can pose a challenge when devices must also consume minimal power.

The \ac{LoRa} wireless communication protocol addresses this challenge by providing a long-range and low-power usage solution, despite having limited throughput.
...
Although \acl{LRED} usually reach much shorter distances in reality, they can still effectively transmit data over multiple kilometers while consuming little power.
...
Although \aclp{LRED} can include \ac{GPS} receivers for geolocation purposes, they consume a significant amount of power, particularly during location acquisition.
Battery-powered devices deployed in remote areas are expected to operate for several years on a single battery; therefore, it is especially problematic in this scenario.
Geolocation of \aclp{LRWED} is also possible without the use of \ac{GPS} receiver by utilizing the signals they send to the \aclp{LRWGW} in the network.

This thesis aims to evaluate and compare \ac{GPS}-less methods for the geolocation of unaltered \aclp{LRWED} and \aclp{LRWGW}.
The goal is to assess techniques that allow scaling up the solution to an entire \ac{LoRaWAN} network.
...
To achieve this objective, some \aclp{LRWGW} already registered in \acf{TTN} in Furtwangen were used, along with new \aclp{GW} installed in Furtwangen.
