...
The other \aclp{LRWGW} that were purchased for the thesis were two Dragino DLOS8N outdoor \aclp{GW}.
...
\subsubsection{ELV LW experimental platform: modular and flexible}\label{subsubsec:elv-lw-experimental-platform}
...
Docker Compose was then used to assemble the different parts of the application: the backend, frontend, \acl{PSQL} \ac{DB} and pgadmin, an admin interface to make viewing \acl{PSQL} data easier.
\Cref{pic:docker-desktop} shows the running containers of the \ac{TTNL} application in the Docker Desktop application~\cite{docker_inc_download_2021}.
...
% It allows for a user to use \ac{VSC} instead of having to rely on the Arduino IDE.

% PlatformIO was used in this thesis to develop code for some of the \aclp{LRWED} used for data collection, for example for the Heltec HTCC-AB02S mentioned in \Cref{subsubsec:heltec-htcc-ab02s}.
...
Vuetify is a \ac{UI} framework for Vue.js that provides Vue.js components that are based on Google's Material Design guidelines~\cite{vuetify_vuetify_2023}~\cite{google_llc_material_nodate}.
In this thesis, Vuetify was used as a modern looking \ac{UI} framework for the frontend of the \ac{TTNL} web application.
Its usage as import-able predefined Vue.js components also allowed for a quick and easy implementation of the frontend.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{pictures/ttn-locator/frontend/vuetify-form-example.png}
    \caption[Example of a form created using Vuetify components]{
...
\subsubsection{Open-source maps for the \acl{TTNL} frontend: \acl{OSM}}\label{sec:osm}
...
It was used in this thesis to display a map showing geolocation data in the frontend of the \ac{TTNL} application.
...
\Cref{fig:rssi-range-graph-positive-example} shows an example of a \ac{RSSI} to distance graph in the frontend of \ac{TTNL}.
...
\subsubsection{Plotting \acl{SNR} to range from existing data}\label{sec:plotting-snr-to-range-from-existing-data}
