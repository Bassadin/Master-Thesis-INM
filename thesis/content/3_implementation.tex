...
This seems to be a hardware problem, since other users could get fixes reliably with external \ac{GPS} antennas.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.6\textwidth]{pictures/hardware/gps-nodes/HTCC-AB02S.jpg}
    \caption{
...
PlatformIO enables the development of embedded systems with \ac{VSC}~\cite{platformio_platformio_nodate}.
...
Vitest is a testing framework for \ac{JS} and \ac{TS}~\cite{vitest_team_vitest_2023-1}.
...
This was used in this thesis to display some data from the \ac{TTNL} frontend on the map.
...
% TODO: mention how thic could improve accuracy
...
An \ac{REST} endpoint was added to the backend that accepts a list of gateway IDs along with maximum and minimum \ac{RSSI} values for those particular gateways.

\begin{lstlisting}[
    language=TypeScript,
    float,
    caption={
        Part of the Prisma query that filters the fingerprint data points by their similarity to the \ac{RSSI} values of gateways provided by the frontend.
    },
...
\begin{figure}[htbp]
    ...
    \caption{
        The frontend of \ac{TTNL} showing the form for entering gateway fingerprinting values.
        ...
        A ``RSSI'' checking range of 3 has been set, meaning that the entered \ac{RSSI} values will be checked against the \ac{RSSI} values of the \ac{TTNM} datapoints with a tolerance of 3.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{pictures/ttn-locator/frontend/rssi_similarity_map_example.png}
    \caption{
    ...
    }\label{fig:rssi-similarity-map-example}
\end{figure}

% TODO: update image with new dynamic radius circle and explain it

\ac{TTNL} can show the matching datapoints on its map as can be seen in \cref{fig:rssi-similarity-map-example}.
The blue pin shows the actual position of the device.
