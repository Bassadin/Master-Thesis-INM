...
\section{Installing \acs{LoRaWAN} gateways in Furtwangen}
...
However, to be able to get a good geolocation through multilateration for a \acl{LRWED}, it is necessary to have at least three gateways in its range, as shown in \cref{sec:multilateration-basics}.
...
\subsection{Possible Locations for \acs{LoRaWAN} gateways}\label{sec:gateway-locations}
...
It also includes the \aclp{LRWGW}, devices and other parts such as the \ac{TTN} \ac{LNS} in the explanation of the data flow.
...
Since, for the scope of this thesis, only devices that collected data near the Furtwangen area were of interest, only data from those devices was transferred.
...
New devices can be added to the \ac{TTNL} \ac{DB} by using a specific backend \ac{API} route.
...
The cronjob for importing new data only does this for devices that are marked as ``subscribed'' in the \ac{DB} to avoid flooding the \ac{DB} with irrelevant data.
...
\section{Locating \acs{LoRaWAN} devices}
...
The \ac{RSSI} value measured by a gateway is inversely proportional to the distance between the \acl{ED} and the gateway.
The further away a \acl{ED} is from a gateway, the lower (or more negative) the \ac{RSSI} value will be.
...
\begin{figure}[htbp]
    ...
    \caption{
        ...
        The \acl{ED} (orange pin) is located at the intersection of the three circles originating from the gateways (green pins).
...
\ac{RSS} values that are received by the gateways are inversely proportional to the distance between the \acl{ED} and the gateway.
They are used to calculate the distance between the \acl{ED} and the gateway, making a multilateration possible.
...
Based on these facts, several attempts were made to calculate the distance between a \acl{ED} and a gateway.
...
The first idea for determining the distance between a \acl{ED} and a gateway was to use a fixed \ac{RSSI} to range scale.
...
The values for this equation were derived by performing a linear regression on the \ac{RSS} values of the \acl{ED}-based GPS data points from the MikroTik LR8 gateway deployed on top of the \ac{GHB} 2 building.
While those values work reasonably well for the \ac{LoS} reception behavior of the gateway that was used to generate the regression, they are not necessarily applicable to other gateways.
...
Another problem, as mentioned in \cref{sec:rssi-based-multilateration}, is that the \ac{MPP} phenomenon can cause the \ac{RSS} values to fluctuate, even when \acl{ED}, gateway, and the distance between them stay the same.
This makes RSSI values in general a rather unreliable metric for determining the distance between a \acl{ED} and a gateway as long as there is no \ac{LoS} between them.
...
It uses the \ac{RSS} values of the \acl{ED}-based data points from a single gateway to generate the graph by calculating the distance to said gateway.
...
As has been explained in \cref{sec:toa-based-multilateration}, this technique uses the time of arrival of a \ac{LoRa} signal to multiple gateways to calculate the position of the \acl{ED} that sent the signal using the speed of light.
