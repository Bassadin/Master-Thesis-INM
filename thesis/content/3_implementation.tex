...
\begin{figure}
    \centering
    \includegraphics[width=1\textwidth]{pictures/hardware/gateway-deployment/gateway_deployment_map.png}
    \caption{Overview map of \ac{LoRaWAN} gateways situated in the Furtwangen area. Shows Gateways placed during this thesis as well as already existing ones.\label{pic:gateways-placement-map}}
\end{figure}
...
\begin{figure}
    ...
    \caption{Antenna with 8dBi gain installed on top of the \emph{DL0FIS} amateur radio club clubhouse (bottom right)\label{pic:antenna-dl0fis-clubhouse}}
\end{figure}
...
% TODO: is this necessary? Copilot, extensions, etc.
% Maybe GitHub actions, etc.
...
It, for example, does not accept datapoints that have a \ac{HDOP} value of more than a certain unspecifíed amount.
...
For example, \lstinline|LAG(latitude) OVER (PARTITION BY "deviceId" ORDER BY timestamp AS prev_latitude| gets the latitude of the previous \ac{GPS} data point of the same device.
...
\begin{figure}
    ...
    \caption{Positive example of a \ac{RSSI} to distance graph the frontend of \ac{TTNL} can generate from data in its \ac{DB}.\label{fig:rssi-range-graph-positive-example}}
\end{figure}
...
\begin{figure}
    ...
    \caption{Negative example of a \ac{RSSI} to distance graph.\label{fig:rssi-range-graph-negative-example}}
\end{figure}
